 ...
 */
public class ExpressionParameterExtractor {
	...
	public static List<String> extractParameters(String expression) {
		...
		Stream<String> parameterStream = parameterList.stream();

		return parameterStream.filter(
			parameter -> Validator.isNotNull(parameter)
		).collect(
			Collectors.toList()
		);
	}
